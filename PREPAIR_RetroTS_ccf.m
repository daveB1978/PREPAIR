...
% adapted from AFNI subroutine RetroTS
...
%create option copy for each type of signal
   OptR = Opt;
      ...
      OptR.AmpPhase = 1;   %amplitude based phase for respiration test 
...
if (~isempty(R)),
   ...
   reg = Opt.Rreg;
   R = PREPAIR_PhaseEstimator(R,OptR,reg);
end
if (~isempty(E)),
   %fprintf(2, 'Estimating phase for E\n');
   reg = Opt.Creg;
   E = PREPAIR_PhaseEstimator(E,OptE,reg);
end
