function [R] = PREPAIR_PhaseEstimator(R, Opt, reg)
% Adapted from AFNI subroutine PhaseEstimator
% Modification: enable to compute more than 2 regressors 
...
for (icol=1:1:length(R)),
   ...
   for (isl=1:1:Opt.Nslices),
      ...
      R(icol).phz_slc_reg(:,2*(j-1)+1, isl) = sin(j.*R(icol).phz_slc(:,isl));
      R(icol).phz_slc_reg(:,2*j, isl) = cos(j.*R(icol).phz_slc(:,isl));
      end
